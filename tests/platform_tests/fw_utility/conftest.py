...
def component_firmware(request, duthost, component_object):
    ...
    yield component_object.process_versions(duthost, binaries_path)
...
def backup_platform_file(duthost):
    ...
    res = duthost.fetch(src=platform_comp_path, dest=backup_path, flat='yes')

    yield

    duthost.copy(src=backup_path, dest=platform_comp_path)


@pytest.fixture(scope='function')
def setup_images(request, duthost, platform_components, component_object, component_firmware):
    """"
    ...
    duthost.copy(src=component_firmware['latest_firmware'], dest=fw_path)
    ...
    if new_current_image == next_image:
        duthost.command("rm -f {}".format(fw_path))
        duthost.copy(src=next_backup_path, dest=PLATFORM_COMP_PATH_TEMPLATE.format(platform_type))
        reboot_to_image(request, image_type=current_image)
        duthost.copy(src=current_backup_path, dest=PLATFORM_COMP_PATH_TEMPLATE.format(platform_type))
    else:
        duthost.copy(src=current_backup_path, dest=PLATFORM_COMP_PATH_TEMPLATE.format(platform_type))
        reboot_to_image(request, image_type=next_image)
        duthost.command("rm -f {}".format(fw_path))
