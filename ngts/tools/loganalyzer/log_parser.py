...
class DutLogAnalyzer:
    ...
    DutLogAnalyzer will be called initially before any test has ran, and will be
    instructed to place 'start' marker into all log files to be analyzed.
    When tests have ran, DutLogAnalyzer will be instructed to place end-marker
    into the log files. After this, DutLogAnalyzer will be invoked to perform the
            ...
            with open(filename, 'r') as csvfile:
                ...
                for index, row in enumerate(csvreader):
                    ...
                    try:
                        ...
                        print('ERROR: line {} is formatted incorrectly in file {}. Skipping line'.format(index, filename))
                        print(repr(e))
            ...
            if not stdin_as_input:
                if rev_line.find(start_marker) != -1 and 'nsible' not in rev_line:
                    ...
                    if(not in_analysis_range):
                        print('ERROR: found start marker:{} without corresponding end marker'.format(rev_line))
...
def main(argv):
    ...
    analyzer = DutLogAnalyzer(run_id, verbose, start_marker)
